...
export function EventTimeline({ events, currentMinute }: EventTimelineProps) {
  const visibleEvents = events.filter((event) => currentMinute >= event.minute);

  return (
    <motion.section
      className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-[22px] border border-white/80 bg-white/82 p-2.5 shadow-lg shadow-slate-200/60 backdrop-blur"
